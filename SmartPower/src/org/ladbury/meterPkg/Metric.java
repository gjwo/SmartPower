...
public class Metric	implements	Serializable,
	...
	public static enum MetricType {UNDEFINED, POWER, ENERGY_LOW_RES, ENERGY_HIGH_RES, POWER_REAL_STANDARD, POWER_REAL_FINE, POWER_REACTIVE_STANDARD};
    ...
    */
    public  void squelchTransitions() {
		...
		//SmartPower.getMain().getFrame().displayLog("interval = "+ interval+ "ms\n\r");
     	for (int i = this.size()-2; i>2; i--){
    		if( this.getIntervals(i)<=Processing.getSquelchThreshold()){
    			// we have a transition or isolated spike
    			// in the future we should record this in a new data structure
    			
     			// to qualify as a plateau the interval at a given value must be > 1 intervals
    			// now check i-1 and i+1 qualify as plateaus
    			if ((this.getIntervals(i-1)>1) &&
    				(this.getIntervals(i+1)>1)){
    				// we have two plateaus surrounding a singleton
    				// eliminate the anomaly by singleton and adding 1 interval to the greater of the surrounding plateaus
    				...
    				} //else by removing the record we have already extended the time intervals of the previous record
    			}
    		}
     	}
    }
	 ...
	 * @return How many ticks have passed since the previous reading
	 */
	public int getIntervals(int row){
		int interval; 
		Timestamp ts = new Timestamp(0);
		if (row < 1 || row >= readings.size()) return 0;
		
		ts.setTime(readings.get(row).timestamp().getTime() -readings.get(row-1).timestamp().getTime());
	...
	public boolean removeRecord(int row){
		...
		if (readings.remove(row)==null){ 
			SmartPower.getMain().getFrame().displayLog("row["+row+"] remove failed\n\r");
			return false;
		}
		if (SmartPower.getMain().getData().getTimedRecords().remove(row)==null){
			SmartPower.getMain().getFrame().displayLog("TimedRecords.remove failed row["+row+"]\n\r");
			return false;
		}
		//SmartPower.getMain().getFrame().displayLog("row["+row+"] removed\n\r");
		return true;
	}
