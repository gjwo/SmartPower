...
public class UiCatEntryTablePanel extends UiDataTablePanel{
	
	private static final long serialVersionUID = 201L;
	
	private UiCatEntryTableLink	model; //this must change when replicating to match the object link table
	private TableRowSorter<UiCatEntryTableLink> sorter;
	
	public UiCatEntryTablePanel(ActionListener al){
		super(al);
		model = new UiCatEntryTableLink();
		getTable().setModel(model);
		sorter = new TableRowSorter<UiCatEntryTableLink>(model);
		getTable().setRowSorter(sorter);
		//{"ID", "Make", "Model",  "Version","Devices", "Select"}
	...
	public void cellSelect(JTable target, int row, int col) {
		...
		if (col == 0){ //the ID column
			...
			SmartPower.getMain().getFrame().displayFormDialogue(SmartPower.getMain().getFrame(),EntityType.CATALOGUE,row); //fire up a form dialogue
			repaint();
		}
	}
